...
function startMethod(){
  ...
  // c) 요청설정 : open()
  xhrObj.open('Get','n12_Promise01.html','true');
...
function startMethodG( ) {
	...
	let url='n13_03parameterResult.html?'+getParameterValues();
	// n09_03parameterResult.html?id=banana&password=12345!&name=바나나 ->요런 url이 아래에 들어감 
...
function startMethodP( ) {
	...
	xhrObj.onreadystatechange=resultF;
	// => 요청설정
	let url='n13_03parameterResult.html';
	xhrObj.open('Post',url,'true'); //Get으로 바꾸면 정상작동되지만 헷갈리니까 나중에 확인 시에만 바꾸기 
...
// => jQuery 구문과 비교해보세요 ~~
/* window.jQuery 윈도우의 객체, ajax라는 메서드를 사용하지 않는 것임 
// jQuery 자주 사용해서 $ 기호로 바꿔줌 
$.ajax({
	  		type:'Get',  // Get/Post
